...
class rankine():
    ...
    def calc_efficiency(self):
        ...
        self.state3 = self.steam.set(self.p_low, x=0, name='Pump Inlet')
    ...
    def plot_cycle_XY(self, X='s', Y='T', ax=None, logx=False, logy=False):
        ...
        # add axis labels
        ax.set_ylabel(self.lowerCurve.getAxisLabel(Y), fontsize='large' if QTPlotting else 'medium')
        ax.set_xlabel(self.lowerCurve.getAxisLabel(X), fontsize='large' if QTPlotting else 'medium')
        ...
        xmin = min(min(XF), min(XG), min(self.upperCurve.getDataCol(X)), max(self.lowerCurve.getDataCol(X)))
        xmax = max(max(XF), max(XG), max(self.upperCurve.getDataCol(X)), max(self.lowerCurve.getDataCol(X)))
        ymin=min(min(YF), min(YG), min(self.upperCurve.getDataCol(Y)), max(self.lowerCurve.getDataCol(Y)))
        ymax=max(max(YF), max(YG), max(self.upperCurve.getDataCol(Y)), max(self.lowerCurve.getDataCol(Y)))*1.1
        ax.set_xlim(xmin,xmax)
        ax.set_ylim(ymin,ymax)
        deltax=xmax-xmin
        deltay=ymax-ymin
        # add the summary text to the plot
        ax.text(xmin+0.05*deltax, ymin+0.7*deltay, self.get_summary())
