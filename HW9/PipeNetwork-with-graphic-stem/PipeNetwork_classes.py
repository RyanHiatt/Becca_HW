...
class Position:
    def __init__(self, pos=None):
        #unpack position if given
        x, y, z = pos if pos is not None else (0.0,0.0,0.0)
        self.x=x
        self.y=y
        self.z=z
    ...
    def set(self,strXYZ=None, tupXYZ=None):
        ...
        if strXYZ is not None:
            ...
            x, y, z = float(cells[0]), float(cells[1]), float(cells[0])
            self.x=float(x)
            self.y=float(y)
            self.z=float(z)
...
class Node: #$NEW$ 4/6/21 got rid of z and added position variable
    def __init__(self, name=None, pipes=None, ext_flow=None, specifiedP=None, min_ph=None, position=None, oldnode=None):
        """
...
class SprinklerHead(Node):
    def __init__(self, name=None, pipes=None, ext_flow=None, position=None, specifiedP=None, min_ph=None, k=None, oldnode=None, SI=True):
        """
        ...
        if oldnode!=None:
            ...
            #run parent constructor
            super().__init__(name=name, pipes=pipes,ext_flow=ext_flow, position=position, specifiedP=specifiedP, min_ph=min_ph)
        self.E=ext_flow if ext_flow is not None else self.E
...
class Loop():
    ...
    def getLoopHeadLoss(self, nodes):
        ...
        for p in self.pipes:
            ...
            deltaZ = nn_o.position.z-sn_o.position.z
...
class PipeNetwork():
    ...
    def recalcPipeLengths(self, Riser=False):
        ...
        """
        for p in self.pipes:
            n1 = self.getNode(p.startNode)
            n2 = self.getNode(p.endNode)
            x1 = n1.position.x
            y1 = n1.position.y
            x2 = n2.position.x
            y2 = n2.position.y
            p.length = ((x2 - x1) ** 2 + (y2 - y1) ** 2) ** 0.5
...
class PipeNetworkController():
    ...
    # def updateNodePipes(self, PN=None):
    #     if PN is not None:
    #         for n in PN.nodes:
    #             n.getNodePipes()
    ...
    def modifyPipe(self, item, col=0, PN=None):
        ...
        if TF:
            ...
            p.diam = float(diam)
            p.modifyPipe(start=stNode, end=enNode, length=float(length), dia=float(diam), roughness=float(rough), fluid=Fluid(), SI=PN.SI)
        else:
        ...
        if type(item) is qtw.QTreeWidgetItem:
            ...
            if TF:
                ...
                if kwargs.get('makesprinkler'):
                    ...
                    PN.nodes[index] = SprinklerHead(oldnode=oldnode, ext_flow=extFlow,  min_ph=minPH)
                else:
                    extFlow=kwargs.get('ext_flow')
                    specPH=kwargs.get('specified_PH')
                    PN.nodes[index] = Node(oldnode=oldnode, ext_flow=extFlow, specifiedP=specPH)
            else:
                if kwargs.get('makesprinkler'):
                    extFlow = kwargs.get('ext_flow')
                    minPH = kwargs.get('min_ph')
                    position=kwargs.get('position')
                    PN.nodes.append(name=name, ext_flow=extFlow, position=position, min_ph=minPH)
                else:
                    extFlow = kwargs.get('ext_flow')
                    specPH = kwargs.get('specified_PH')
                    position=kwargs.get('position')
    ...
    def modifyNode_TreeItem(self, item, col=0, PN=None):
        ...
        cols = i.columnCount()
        p = [i.text(c) for c in range(cols)]
        name, x,y,z,extflow, issprinkler, minph = p  #$NEW$ 4/6/21 modified this to take x,y,z arguments
        TF = PN.hasNode(name)
    ...
    def importPipe(self, data, j, PN=PipeNetwork()):
        ...
        while L.find('/pipe')==-1:
            ...
            if cells[0].lower().find('nodes')>=0:
                ...
                P.setLength(float(cells[1].strip()), PN.SI)
            elif cells[0].lower().find('dia')>=0:
                P.setDiam(float(cells[1].strip()), PN.SI)
            elif cells[0].lower().find('rough')>=0:
                P.setRough(float(cells[1].strip()), PN.SI)
            j+=1
            L=data[j].lower()
...
class PipeNetworkView():
    ...
    def updatePipeTree(self, tree, PN=None):
        tree.clear()
        for p in PN.pipes:
            itm = qtw.QTreeWidgetItem((p.getName(), str(round(p.length,2)), str(p.diam), str(p.rough)))
    ...
    def updateNodeTree(self, tree, PN=None):
        ...
        PN.buildNodes()
        for n in PN.nodes:
            itm = qtw.QTreeWidgetItem((n.getName(), str(n.position.x), str(n.position.y), str(n.position.z), str(n.E), str(n.IsSprinkler), str(n.MinPH if n.IsSprinkler else n.SpecifiedP)))
            itm.setFlags(qtc.Qt.ItemIsSelectable | qtc.Qt.ItemIsEditable | qtc.Qt.ItemIsDragEnabled | qtc.Qt.ItemIsUserCheckable | qtc.Qt.ItemIsEnabled)
            tree.addTopLevelItem(itm)
    ...
    #region $NEW$ 4/6/21 functions for drawing the pipe network
    def drawNodes(self, PN=None, scene=None, penNode=None, brushNode=None):
    ...
    def drawPipes(self, PN=None, scene=None, penPipe=None, brushPipe=None):
        for p in PN.pipes:
            ...
            x2 = n2.position.x
            y2 = n2.position.y
            self.drawAnArrow(x1, y1, x2, y2, pen=penPipe, brush=brushPipe, scene=scene)

    def drawLoopLabels(self, PN=None, pen=None, scene=None):
        loop_center = {}
        for l in PN.loops:
            nodes = []
            x_coords = []
            y_coords = []
            for p in l.pipes:
                if p.startNode not in nodes:
                    nodes.append(p.startNode)
                    x_coords.append(PN.getNode(p.startNode).position.x)
                    y_coords.append(PN.getNode(p.startNode).position.y)
                if p.endNode not in nodes:
                    nodes.append(p.endNode)
                    x_coords.append(PN.getNode(p.endNode).position.x)
                    y_coords.append(PN.getNode(p.endNode).position.y)

            self.drawALabel(statistics.mean(x_coords), statistics.mean(y_coords), f'{l.name}', pen=pen, scene=scene)

    def drawNodeExtFlows(self, PN=None, scene=None, pen=None, brush=None):
        for n in PN.nodes:
            x = n.position.x
            y = n.position.y
            if n.E >= 0:
                self.drawAnArrow(x + 25, y + 25, x + 5, y + 5, pen=pen, brush=brush, scene=scene)
                self.drawALabel(x + 30, y + 35, str=f'{n.E}', pen=pen, scene=scene)
            else:
                self.drawAnArrow(x + 5, y + 5, x+20, y+20, pen=pen, brush=brush, scene=scene)
                self.drawALabel(x+30, y+35, str=f'{n.E}', pen=pen, scene=scene)


    def polarToRect(self, centerX, centerY, radius, angleDeg=0.0, scene=None):
        angleRad = angleDeg * 2.0 * math.pi / 360.0
        return centerX + radius * math.cos(angleRad), centerY + radius * math.sin(angleRad)

    def drawACircle(self, centerX, centerY, Radius, angle=0, brush=None, pen=None, scene=None):
    ...
    def drawASquare(self, centerX, centerY, Size, brush=None, pen=None, scene=None):
    ...
    def drawATriangle(self, centerX, centerY, Radius, angleDeg=0.0, brush=None, pen=None, scene=None):
        pts = []
    ...
    def drawAnArrow(self, startX, startY, endX, endY, pen=None, brush=None, scene=None):
        line = qtw.QGraphicsLineItem(startX, startY, endX, endY)
        p = qtg.QPen() if pen is None else pen
        line.setPen(p)
        angleDeg = 180.0 / math.pi * math.atan((endY - startY) / (endX - startX + 0.000001))
        scene.addItem(line)
        self.drawATriangle(endX, endY, 5, angleDeg=angleDeg, pen=pen, brush=brush, scene=scene)

    def drawALabel(self, x,y,str='', pen=None, scene=None):
    ...
    def getPipeFlowRatesOutputSorted(self, SI=True, tabs='', PN=None, lbl=None, tree=None):
        stOut = 'Flow Results:\n'
        items=tree.topLevelItemCount()
        for i in range(items):
            p=PN.getPipe(tree.topLevelItem(i).text(0))
            stOut += p.getPipeFlowRateOutput(SI=SI, tabs=tabs) + '\n'
        if not lbl is None:
            lbl.setText(stOut)
        return stOut
    ...
    def getNetNodeFlowsOutputSorted(self, SI=True, tabs='', PN=None, lbl=None, tree=None):
        stOut = 'Net Flow Into Nodes:\n'
        items=tree.topLevelItemCount()
        for i in range(items):
            n=PN.getNode(tree.topLevelItem(i).text(0))
            Q = n.getNetFlowRate()
            Q = Q if SI else Q / 1000 * (3.3 ** 3)
            q_units = 'L/s' if SI else 'cfs'
            stOut += tabs + 'Q into {} = {:0.2f} {}'.format(n.getName(), n.getNetFlowRate(), q_units) + '\n'
    ...
    def getLoopHeadLossOutputSorted(self, SI=True, tabs='', PN=None, lbl=None, tree=None):
        stOut = 'Loop Head Losses:\n'
        items=tree.topLevelItemCount()
        for i in range(items):
    ...
    def getPipeHeadLossesOutputSorted(self, SI=True, tabs='', PN=None, lbl=None, tree=None):
        stOut = 'Pipe Head Losses:\n'
        items = tree.topLevelItemCount()
        for i in range(items):
            p=PN.getPipe(tree.topLevelItem(i).text(0))
    ...
    def getNodeHeadOutputSorted(self, SI=True, tabs='', PN=None, lbl=None, tree=None):
        stOut = 'Node Pressures\n'
        items = tree.topLevelItemCount()
        for i in range(items):
            N = PN.getNode(tree.topLevelItem(i).text(0))
            ...
            if N.IsSprinkler:
                stOut += tabs + 'PH at sprinkler {} (Z={:0.2f}m) = {:0.2f} {} (K={:0.2f}, Q={:0.2f} L/S)'.format(
                    N.getName(), N.position.z, p, p_units, N.K, abs(N.E)) + '\n'
            else:
                stOut += tabs + 'PH at node {} (Z={:0.2f}m) = {:0.2f} {}'.format(N.getName(), N.position.z, p, p_units) + '\n'
        if not lbl is None:
            lbl.setText(stOut)
        return stOut

    def getRealityCheckOutputSorted(self, SI=True, tabs='', PN=None, lbl=None, treeN=None, treeL=None):
        lblA=qtw.QLabel()
        lblB=qtw.QLabel()
        self.getLoopHeadLossOutputSorted(SI=SI,tabs='\t', PN=PN, lbl=lblA, tree=treeL)
        stOut=lblA.text()
        stOut += '\n\n'
        self.getNetNodeFlowsOutputSorted(SI=SI,tabs='\t', PN=PN, lbl=lblB, tree=treeN)
        stOut+=lblB.text()
        if not lbl is None:
            lbl.setText(stOut)
        return stOut
...
#                        lb/ft^3 for specific gravity\n'
    ...
    def getPipeNetworkOutputForFileSorted(self, filename=None, PN=None, treeP=None, treeN=None, treeL=None):
            header = '# Pipe Network\n\
    # The SI units are: m for pipe length, mm for pipe diameter\n\
    #                  L/s for external flow rates\n\
    #                  m of water for pressure head\n\
    ...
    #                        lb/ft^3 for specific gravity\n'
            ...
            for i in range(treeL.topLevelItemCount()):
                ...
                stTmp += '\n'
                stTmp += '</Loop>\n'
            for i in range(treeN.topLevelItemCount()):
                n = PN.getNode(treeN.topLevelItem(i).text(0))
                if n.E > 0.0 or n.E < 0.0:
                    stTmp += '<Node>\n'
                    stTmp += '\tName: {}\n'.format(n.getName())
                    stTmp += '\tExternal Flow: {:0.1f}\n'.format(n.E)
    ...
    def getPipeNetworkOutputForFileSortedUsingNodes(self, filename=None, PN=None, treeP=None, treeN=None, treeL=None):
            header = '# Pipe Network\n\
    # The SI units are: m for pipe length, mm for pipe diameter\n\
    #                  L/s for external flow rates\n\
    #                  m of water for pressure head\n\
    ...
    #                        lb/ft^3 for specific gravity\n'
            ...
            for i in range(treeN.topLevelItemCount()):
                ...
                if n.E > 0.0 or n.E < 0.0:
                    stTmp += '<Node>\n'
                    stTmp += '\tName: {}\n'.format(n.getName())
                    stTmp += '\tPosition: {}, {}, {}\n'.format(n.position.x, n.position.y, n.position.z)
                    stTmp += '\tExternal Flow: {:0.1f}\n'.format(n.E)
                    stTmp += '\tMinP: {}\n'.format(n.MinPH) if n.IsSprinkler else ''
                    stTmp += '\tSprinkler: True\n' if n.IsSprinkler else ''
                    stTmp += '</Node>\n'
...
def mainX2():
    ...
    # sf=1.0
    # PN.getNode('a').position.z= 0.0
    # PN.getNode('b').position.z= 2.5/sf #sprinkler
    # PN.getNode('c').position.z= 5.0/sf
    # PN.getNode('d').position.z= 5.0/sf #sprinkler
    # PN.getNode('e').position.z= 5.0/sf
    # PN.getNode('f').position.z= 4.0/sf #sprinkler
    # PN.getNode('g').position.z= 4.5/sf
    # PN.getNode('h').position.z= 4.0/sf #sprinkler
    #
...
def mainHW6():
    ...
    #now change node elevations (which also changes pipe lengths by geometry assuming straight pipe runs between nodes)
    sf=1.0
    PNC.modifyNode(PN=PN, name='a', position=Position((0,0,0)))
    PNC.modifyNode(PN=PN, name='b', position=Position((0,0,2.5)), makesprinkler=True)
    PNC.modifyNode(PN=PN, name='c', position=Position((0,0,5.0)))
    PNC.modifyNode(PN=PN, name='d', position=Position((0,0,5.0)), makesprinkler=True)
    PNC.modifyNode(PN=PN, name='e', position=Position((0,0,5.0)))
    PNC.modifyNode(PN=PN, name='f', position=Position((0,0,4.0)), makesprinkler=True)
    PNC.modifyNode(PN=PN, name='g', position=Position((0,0,4.5)))
    PNC.modifyNode(PN=PN, name='h', position=Position((0,0,4.5)), makesprinkler=True)
