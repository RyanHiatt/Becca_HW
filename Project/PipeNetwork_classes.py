...
class Units():
    def __init__(self, SI=True):
        ...
        self.CFPressure = (1000.0 / 25.4) /27.7076  # m H2O to psi
...
class PipeNetworkView():
    ...
    def updateNodeTree(self, tree, PN=None):
        ...
        PN.buildNodes()
        for n in PN.nodes:
            itm = qtw.QTreeWidgetItem((n.getName(), str(n.position.x), str(n.position.y), str(n.position.z), str(n.E),
                                       str(n.IsSprinkler), str(n.MinPH if n.IsSprinkler else n.SpecifiedP)))
            itm.setFlags(
                qtc.Qt.ItemIsSelectable | qtc.Qt.ItemIsEditable | qtc.Qt.ItemIsDragEnabled | qtc.Qt.ItemIsUserCheckable | qtc.Qt.ItemIsEnabled)
            tree.addTopLevelItem(itm)
    ...
    def printNodeHead(self, SI=True, PN=None):
        for N in PN.nodes:
            ...
            p_units = 'm of water' if SI else 'psi'
            if N.IsSprinkler:
                print('PH at sprinkler {} (Z={:0.2f}m) = {:0.2f} {} (K={:0.2f}, Q={:0.2f} L/S)'.format(N.Name, N.Z, p,
                                                                                                       p_units, N.K,
                                                                                                       abs(N.E)))
            else:
    ...
    def getNodeHeadOutputSorted(self, SI=True, tabs='', PN=None, lbl=None, tree=None):
        ...
        for i in range(items):
            ...
            if N.IsSprinkler:
                ...
                stOut += tabs + 'PH at node {} (Z={:0.2f}m) = {:0.2f} {}'.format(N.getName(), N.position.z, p,
                                                                                 p_units) + '\n'
        if not lbl is None:
            lbl.setText(stOut)
        return stOut
