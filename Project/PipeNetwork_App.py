...
class MainWindow(qtw.QWidget, Ui_Form):
    ...
    def savePipeNetworkFile(self):
        ...
        if len(filename) == 0:  # no file selected
            ...
            self.View.getPipeNetworkOutputForFileSortedUsingNodes(filename=filename, PN=self.Model,
                                                                  treeP=self.tree_Pipes, treeN=self.tree_Nodes,
    ...
    def addPipeToPipeList(self):
        ...
        self.Controller.addPipe(itm,
                                PN=self.Model)  # part of the controller that updates the model with changes on the view
    ...
    def EvaluatePipeNetwork(self):
        ...
        self.View.getPipeHeadLossesOutputSorted(tabs='\t', PN=self.Model, lbl=self.lbl_PipeHeadLosses,
                                                tree=self.tree_Pipes)
        self.View.getNodeHeadOutputSorted(tabs='\t', PN=self.Model, lbl=self.lbl_NodePressures, tree=self.tree_Nodes)
        self.View.getRealityCheckOutputSorted(tabs='\t', PN=self.Model, lbl=self.lbl_PressureAndFlowChecks,
                                              treeN=self.tree_Nodes, treeL=self.tree_Loops)
        return None
