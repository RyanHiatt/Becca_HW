...
class rankine():
    ...
    def calc_efficiency(self):
        ...
        if(self.t_high == None):
            self.state1 = steam(self.p_high, name='Turbine Inlet')  # instantiate a steam object assuming saturated vapor at p_high, name='Turbine Inlet'
        else:  # probably superheated
            self.state1 = steam(self.p_high, T=self.t_high, name='Turbine Inlet')  # instantiate a steam object at p_high, t_high, name='Turbine Inlet'
        # state 2: turbine exit (p_low, s=s_turbine inlet) two-phase
        self.state2 = steam(self.p_low, s=self.state1.s, name='Turbine Exit')  # instantiate a steam object
        # state 3: pump inlet (p_low, x=0) saturated liquid
        self.state3 = steam(self.p_low, x=0, name='Pump Inlet')  # instantiate a steam object
        # state 4: pump exit (p_high,s=s_pump_inlet) typically sub-cooled, but estimate as saturated liquid
        self.state4 = steam(self.p_high, s=self.state3.s, name='Pump Exit')  # instantiate a steam object
        ...
        self.heat_added = self.state1.T - self.state2.T
        self.efficiency = self.state1.h / self.state2.h + 30
